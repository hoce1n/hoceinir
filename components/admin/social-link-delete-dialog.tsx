...
import { deleteSocialLink } from "@/app/admin/(shell)/messages/actions"
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
      ...
      <AlertDialogContent className="border-border bg-card font-mono">
        <AlertDialogHeader>
          <AlertDialogTitle className="font-mono text-foreground">
            <span className="text-destructive">$ rm -rf</span> social/
            {socialName}
          </AlertDialogTitle>
          <AlertDialogDescription className="font-mono text-xs leading-relaxed text-muted-foreground">
            This removes <span className="text-foreground">{socialName}</span>{" "}
            from the public contact endpoints. This operation cannot be undone.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
