...
import { deleteUsesGroup } from "@/app/admin/(shell)/uses/actions"
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
type UsesDeleteDialogProps = {
  usesGroupId: string
  groupName: string
}

export function UsesDeleteDialog({
  usesGroupId,
  groupName,
}: UsesDeleteDialogProps) {
  ...
  const handleDelete = async () => {
    setIsDeleting(true)
    const result = await deleteUsesGroup(usesGroupId)
    setIsDeleting(false)
      ...
      <AlertDialogContent className="border-border bg-card font-mono">
        <AlertDialogHeader>
          <AlertDialogTitle className="font-mono text-foreground">
            <span className="text-destructive">$ rm -rf</span> ./{groupName}
          </AlertDialogTitle>
          <AlertDialogDescription className="font-mono text-xs leading-relaxed text-muted-foreground">
            This permanently removes{" "}
            <span className="text-foreground">{groupName}</span> from the Uses
            registry. This operation cannot be undone.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
